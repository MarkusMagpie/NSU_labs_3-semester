...
void SoundProcessor::MainProcess() {
    std::vector<std::vector<int16_t>> input_streams; // Вектор для хранения ПОТОКОВ (тоже векторов) всех входных WAV-файлов
    ...
    // в методе Parse() заполняем вектор из указателей на конвертеры converters
    const std::vector<std::unique_ptr<Converter>>& converters = parser.GetConverters(); // передаем данные converters локальной переменной
