...
std::vector<int16_t> AmplifyConverter::Convert(const std::vector<int16_t>& input) {
    ...
    std::cout << "Amplify all samples by factor " << amplification_factor << " between " << start_time << " and " << end_time << " seconds" << std::endl;
    ...
    for (double i = start; i < end; i += 1.0) {
        ...
        output[i] = static_cast<int16_t>(std::min(std::max(amplified_sample, INT16_MIN), INT16_MAX));
